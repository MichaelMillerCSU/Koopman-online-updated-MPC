...
RMSE = np.linalg.norm((test_Y[0, :] - X[0, 0: plotTime]) / plotTime, ord = 2)

print("\nRMSE : ", RMSE)
scio.savemat('DuffingPlot_trajectory.mat', {'tspan': tspan, 'tspan_pred':tspan_pred, 'X':X, 'Y':Y, 'U':U, 'test_Y':test_Y, 'marker_T':marker_T, 'marker_originX':marker_originX, 'test_X':test_X, 'decoder_X':decoder_X, 'marker_X':marker_X, 'Uplot':Uplot})

plt.figure()
for i in range (0, n):
    plt.subplot(n, 1, i + 1)
# plt.title('$x_2$ and $x_2$ reconstruction')
    plt.plot(tspan, inputs_x[i, 0: plotTime],linestyle='--', label = '$x_' + str(i + 1) + '$ true')
# plt.plot(tspan, test_Y[1, :],marker='.', label = "reconstructed $x_2$ trajectory ")
    plt.plot(tspan_pred, test_Y[i, :], label = '$\hat{x}_' + str(i + 1) + '$ predict')
...
#         phix = torch.squeeze(phix)
#         marker_x = net.Decoder(phix)
#         marker_x = C @ phix.detach().numpy()
#         marker_x = torch.tensor(marker_x)
#         marker_x = torch.reshape(marker_x, (len(marker_x), 1))
#         marker_X = torch.cat([marker_X, marker_x], dim = 1)
#         marker_T.append(i * h)
#
...
#     phix = torch.squeeze(phix)
#     decoder_x = net.Decoder(phix)
#     decoder_x = C @ phix.detach().numpy()
#     decoder_x = torch.tensor(decoder_x)
#     decoder_x = torch.reshape(decoder_x, (len(decoder_x), 1))
#
#     #decoder_x = np.reshape(decoder_x, (len(decoder_x), 1))
#     phix = torch.reshape(phix, (len(phix), 1))
